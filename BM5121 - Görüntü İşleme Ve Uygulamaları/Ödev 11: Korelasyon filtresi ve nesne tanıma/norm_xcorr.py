...
class TemplateMatch(object):
    ...
    def __init__(self, template, method="ssd"):
        ...
        if method not in ["ncc", "ssd", "both"]:
            raise Exception(
                'Invalid method "%s". '
                'Valid methods are "ncc", "ssd" or "both"' % method
            )
    ...
    def __call__(self, a):
        ...
        if a.ndim != self.template.ndim:
            raise Exception(
                "Input array must have the same number "
                "of dimensions as the template (%i)" % self.template.ndim
            )
...
def norm_xcorr(t, a, method=None, trim=True, do_ssd=False):
    ...
    sigma_t = np.sqrt(t.size - 1.0) * std_t
    ...
    nxcorr = np.where(
        np.abs(nxcorr - 1.0) > np.sqrt(np.finfo(nxcorr.dtype).eps), nxcorr, 0
    )
...
def ndpad(a, npad=None, padval=0):
    ...
    *If npad is not a whole number, padding will be applied so that the
    ...
    if npad == None:
        ...
        raise Exception(
            "Length of npad (%i) does not match the "
            "dimensionality of the input array (%i)" % (len(npad), a.ndim)
        )
    ...
    ind = [
        slice(np.floor(npad[dd]), a.shape[dd] + np.floor(npad[dd]))
        for dd in range(a.ndim)
    ]
...
def procrustes(a, target, side="both", padval=0):
    ...
    try:
        if len(target) != a.ndim:
            raise TypeError(
                "Target shape must have the same number of dimensions as the input"
            )
    except TypeError:
    ...
    # pad/trim both sides of the array in each dimension
        ...
        for dd in range(a.ndim):
            if a.shape[dd] > target[dd]:
                diff = (a.shape[dd] - target[dd]) / 2.0
                aind[dd] = slice(np.floor(diff), a.shape[dd] - np.ceil(diff))
            elif a.shape[dd] < target[dd]:
                diff = (target[dd] - a.shape[dd]) / 2.0
