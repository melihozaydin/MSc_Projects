...
def create_model_fcn32(nb_class, input_w=256):
    ...
    x = Conv2D(
        4096, kernel_size=(7, 7), use_bias=False, activation="relu", padding="same"
    )(vgg.output)
    x = Dropout(0.5)(x)
    x = Conv2D(
        4096, kernel_size=(1, 1), use_bias=False, activation="relu", padding="same"
    )(x)
    x = Dropout(0.5)(x)
    x = Conv2D(nb_class, kernel_size=(1, 1), use_bias=False, padding="same")(x)
    # upsampling to image size
    x = Conv2DTranspose(
        nb_class, kernel_size=(64, 64), strides=(32, 32), use_bias=False, padding="same"
    )(x)
