...
def plot_cv_img(input_image, output_image1, output_image2):     
    ...
    # plt.savefig('./images/03_image_derivatives_log.png')
...
def main():
    # read an image 
    img = cv2.imread('./images/png/building2-1.png')
    ...
    plt.axis('off')
    plt.show()
    # Convert floating point images to uint
    lapl = np.asarray(lapl, dtype= np.uint)
    log = np.asarray(log, dtype= np.uint)
    # Do plot
